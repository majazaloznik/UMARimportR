...
BEGIN
    ...
    INSERT INTO test_platform.category_table (table_id, category_id, source_id)
    ...
    JOIN test_platform.series s ON s.id = v.series_id
    WHERE s.table_id = v_table_id;

    -- Add flags
    INSERT INTO test_platform.flag_datapoint (vintage_id, period_id, flag_id)
    SELECT v.id, '2020', 'e'
    FROM test_platform.vintage v
    JOIN test_platform.series s ON s.id = v.series_id
    WHERE s.table_id = v_table_id;
    ...
    SELECT * FROM test_platform.delete_table(v_table_id) INTO v_result;
    ...
    ASSERT NOT EXISTS (SELECT 1 FROM test_platform.series WHERE table_id = v_table_id),
        'No series should remain';
    ASSERT NOT EXISTS (SELECT 1 FROM test_platform.table_dimensions WHERE table_id = v_table_id),
        'No dimensions should remain';
    ASSERT NOT EXISTS (SELECT 1 FROM test_platform.category_table WHERE table_id = v_table_id),
...
BEGIN
    ...
    INSERT INTO test_platform.period (id, interval_id)
    VALUES ('A2020', 'A')
    ...
    -- Test deletion and count verification
    SELECT * FROM test_platform.delete_series(v_series_id) INTO v_result;
    ...
    -- Verify no orphaned records
    ASSERT NOT EXISTS (SELECT 1 FROM test_platform.vintage WHERE series_id = v_series_id),
        'No vintages should remain';
    ASSERT NOT EXISTS (SELECT 1 FROM test_platform.series_levels WHERE series_id = v_series_id),
        'No series_levels should remain';

    -- Test deletion of non-existent series (should return all zeros)
    SELECT * FROM test_platform.delete_series(999999) INTO v_result;
